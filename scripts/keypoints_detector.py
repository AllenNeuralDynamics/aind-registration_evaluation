...
from scipy.ndimage import maximum_filter, gaussian_laplace, gaussian_filter
...
def plot_quiver(ax, x, y, scale=1, n=None, color=None):
    ...
    if color is not None:
        ax.quiver(np.zeros(n), np.zeros(n), x[:n], y[:n], units='xy', scale=scale, color=color)
    else:
...
def kd_fft_keypoints(
    ...
    max_bboxs = kd_compute_bboxs_cubes(max_points, pad_width, response_img.shape)
    min_bboxs = kd_compute_bboxs_cubes(min_points, pad_width, response_img.shape)
...
def kd_fft_energy_keypoints(
    ...
    # Computing bboxs based on window size = pad_width
    max_bboxs = kd_compute_bboxs_cubes(energy_points, pad_width, response_img.shape)
...
def kd_gradient_magnitudes_and_orientations(derivated_images:List[np.array]):
    ...
    if n_dims == 2:
        ...
        theta = np.arccos(derivated_images[0] / gradient_magnitude) # Angle Z - plane XY
...
def kd_compute_keypoints_hog_2(
    ...
    for z_idx in range(cell_size_per_dimension[0]):
        for y_idx in range(cell_size_per_dimension[1]):
            for x_idx in range(cell_size_per_dimension[2]):
                ...
                if n_dims == 2:
                    quadrant = (
                        slice(cell_size * y_idx, cell_size * y_idx + cell_size),
                        slice(cell_size * x_idx, cell_size * x_idx + cell_size)
                    )
                else:
                    quadrant = (
                        slice(cell_size * z_idx, cell_size * z_idx + cell_size),
                        slice(cell_size * y_idx, cell_size * y_idx + cell_size),
                        slice(cell_size * x_idx, cell_size * x_idx + cell_size)
                    )
                ...
                if n_dims == 2:
                    ...
                    hist_data = [cell_orientation_z_xy.flatten(), cell_orientation_yx.flatten()]
                ...
                hogs_per_quadrant[get_quadrant(cell_size, x_idx, y_idx)][(y_idx, x_idx)] = {
                    "wgh": hist_response[0],
                    "wgb": hist_response[1],
                    "img": cell_magnitude
                }

                weighted_gradient_histograms.append(
                    hist_response[0]
                )
                weighted_gradient_bins.append(
                    hist_response[1]
                )
    ...
    for ys in range(cell_size):
        for xs in range(cell_size):
            print(f"pos ({ys}, {xs}) belongs to quadrant {(get_quadrant(cell_size, xs, ys))}")
    ...
    if n_dims == 2:
        ...
        print("SUM: ", sum_of_w_g_h[arg_max_idx], weighted_gradient_histograms[arg_max_idx], arg_max_idx)

        dominant_orientation = [
            arg_max_idx * (360 / bins[0])
        ]
        print("Dominant orientation: ", dominant_orientation)
        print(f"Argmax: {arg_max_idx}")
        
        check = np.min(region_magnitude), np.max(region_magnitude),
        ...
        print("Dominant orientation: ", weighted_gradient_histograms[arg_max_idx], weighted_gradient_bins[arg_max_idx], arg_max_idx)
        print("Resultant vector angle: ", np.rad2deg(np.arctan2(np.rad2deg(y[-1]), np.rad2deg(x[-1]))))
        
        f, ax = plt.subplots(1,2)
        ax[0].imshow(hogs_per_quadrant[3][(3, 3)]["img"])#, vmin=check[0], vmax=check[1])
        plot_quiver(
            ax=ax[1],
            x=x,#hist=weighted_gradient_histograms[arg_max_idx],
            y=y,#bins=weighted_gradient_bins[arg_max_idx],
            scale=1200,
            color=["red", "yellow", "blue", "gray", "purple", "green", "orange", "cyan"]
        )
        plt.show()
...
def kd_compute_keypoints_hog(
    ...
    if n_dims == 2:
        ...
        feature_vector = feature_vector / (np.sqrt(np.sum(feature_vector ** 2)) + 1e-8)
...
def test_fft_max_min_keypoints(img_1, img_2):
    ...
    max_img_1_keypoints, min_img_1_keypoints, filtered_img_1 = kd_fft_keypoints(
        image=img_1,
        pad_width=pad_width
    )
    max_img_2_keypoints, min_img_2_keypoints, filtered_img_2 = kd_fft_keypoints(
        image=img_2,
        pad_width=pad_width
    )
...
def compute_distance(feat_1, feat_2, cartesian_1, cartesian_2):
    ...
    # feats = np.array([feat_1, feat_1])
    # print("Linalg norm: ", np.linalg.norm(feats))
    # print("Distance: ", feat_1 - feat_2)
    feat_2_1_dif = feat_1 - feat_2
    dist = np.power(feat_2_1_dif, 2).sum() + np.abs(cartesian_1 - cartesian_2).sum() #np.sqrt(np.sum(feat_2_1_dif * feat_2_1_dif, axis = 1))
    # print("distance: ", dist.shape, " sum: ", np.sum(dist))
    # print("feature shapes ", feat_1.shape, feat_2.shape)

    distances = {}
    distances["own"] = dist
    for t in ['euclidean', 'seuclidean', 'sqeuclidean', 'sqeuclidean', 'hamming', 'jaccard', 'jensenshannon', 'chebyshev', 'canberra', 'braycurtis', 'mahalanobis', 'yule', 'matching', 'dice', 'kulczynski1', 'rogerstanimoto', 'russellrao', 'sokalmichener', 'sokalsneath']:
        dis = cdist(feat_1, feat_2, t)
        # print(f"distance {t} is {np.sum(dis)}")
        distances[t] = np.sum(dis)
    
    return distances
    

    # C = euc[np.argmin(dist, 1)]
    # take two nearest distances
    # vals = np.sort(dist, 1)[:,:2]

    # print("Cdist: ", euc.shape, np.sum(euc))

def compute_feature_space_distances(features_1:np.array, features_2:np.array) -> np.array:

    # print("Feature shapes: ", features_2.shape, features_1.shape)
    feature_distances = np.empty((features_1.shape[0], features_2.shape[0])) #np.empty((features_1.shape[0], features_2.shape[0]))

    for i in range(features_1.shape[0]):
        feature_2_1_dif = features_2 - features_1[i]
        # print("feat dif: ", feature_2_1_dif.shape)
        distance = np.sqrt(np.sum(feature_2_1_dif * feature_2_1_dif, axis = 1))[0]
        # print(f"Distance: {distance} feat_distance shape {feature_distances[i].shape}")
        feature_distances[i] = distance

    return feature_distances

def compute_nndr_from_features(feature_1, feature_2, nndr_threshold=0.75):

    distances = compute_feature_space_distances(feature_1, feature_2)
    print(distances[0])
    exit()
    distances = torch.from_numpy(distances)


    top2 = torch.topk(distances, 2, dim = 1, largest = False)
    nndrs = top2.values[:, 0] / top2.values[:, 1]
    
    # print("TOP: ", top2, nndrs)

    matches = []
    confidences = []
    for i in range(distances.shape[0]):
        if nndrs[i] <= nndr_threshold:
...
def generate_key_features_per_img2d(img_2d):
    ...
    gradient_magnitude, gradient_orientation, gradient_orientation_polar = kd_gradient_magnitudes_and_orientations(
        derivated_images=[dy_val, dx_val]#[img_2d_dy, img_2d_dx]
    )

    img_keypoints_features = [kd_compute_keypoints_hog(
...
def test_fft_energy_keypoints(img_1, img_2):
    ...
    print(f"N keypoints img_1: {img_1_dict['keypoints'].shape} img_2: {img_2_dict['keypoints'].shape}")
    ...
    # axarr[0].set_title("Original")
    # axarr[0].imshow(img_1)
    # axarr[0].plot(img_1_dict["keypoints"][:, 1], img_1_dict["keypoints"][:, 0], 'r.') # max points

    # axarr[1].set_title("FFT Gauss Laplaced Filtered")
    # axarr[1].imshow(img_1_dict["response_img"])
    # axarr[1].plot(img_1_dict["keypoints"][:, 1], img_1_dict["keypoints"][:, 0], 'r.') # max points
    ...
    # axarr[0].set_title("Original")
    # axarr[0].imshow(img_2)
    # axarr[0].plot(img_2_dict["keypoints"][:, 1], img_2_dict["keypoints"][:, 0], 'r.') # max points

    # axarr[1].set_title("FFT Gauss Laplaced Filtered")
    # axarr[1].imshow(img_2_dict["response_img"])
    # axarr[1].plot(img_2_dict["keypoints"][:, 1], img_2_dict["keypoints"][:, 0], 'r.') # max points
    # plt.tight_layout()
    # plt.show()
    ...
    feat_left = img_1_dict["features"]
    points_left = img_1_dict['keypoints']

    feat_right = img_2_dict["features"]
    points_right = img_2_dict["keypoints"]
    # print(point_left, point_right)
    
    for idx_left in range(len(feat_left)):
        idx_dict = {}
        for feat_move in range(feat_right.shape[0]):
            # print("\nfeat move shape: ", feat_right[feat_move].shape)
            distances = compute_distance(
                feat_left[idx_left],
                feat_right[feat_move],
                points_left[idx_left],
                points_right[feat_move]
            )
            idx_dict[(idx_left, feat_move)] = distances

        print("\n\n")

        for dis_m in ['own', 'euclidean', 'seuclidean', 'sqeuclidean', 'hamming', 'jaccard','chebyshev', 'matching', 'dice']:
            min_key = min(idx_dict, key=lambda key: np.min(idx_dict[key][dis_m]))
            print(f"Best keypair is {min_key} with distance {dis_m}")
    ...
    for idx in range(img_1_dict["keypoints"].shape[0]):
        x, y = img_1_dict["keypoints"][idx][1], img_1_dict["keypoints"][idx][0]
        axarr[0].plot(x, y, 'r.') # max points
        axarr[0].annotate(
            str(idx),
            (x, y),
    ...
    axarr[1].imshow(img_2_dict["response_img"])
    # axarr[1].plot(img_2_dict["keypoints"][:, 1], img_2_dict["keypoints"][:, 0], 'r.') # max points
    for idx in range(img_2_dict["keypoints"].shape[0]):
        x, y = img_2_dict["keypoints"][idx][1], img_2_dict["keypoints"][idx][0]
        axarr[1].plot(x, y, 'r.') # max points
        axarr[1].annotate(
            str(idx),
            (x, y),
            textcoords="offset points",
    ...
    # distances = compute_feature_space_distances(img_1_dict["features"], img_2_dict["features"])
    # print(distances.shape)
    # matches, confidences = compute_nndr_from_features(img_1_dict["features"], img_2_dict["features"])
...
def test_img_3d_orientations(img_3d):
    ...
    dz_val, dy_val, dx_val = derivate_image_axis(
        img_3d, [0, 1, 2]
    )
    ...
    gradient_magnitude_3d, gradient_orientation_yx_3d, gradient_orientation_z_yx_3d = kd_gradient_magnitudes_and_orientations(
        derivated_images=[filtered_img_3d_dz, filtered_img_3d_dy, filtered_img_3d_dx]
    )
    ...
    for keypoint in img_3d_keypoints_energy:
        ...
        img_3d_res = kd_compute_keypoints_hog(
            ...
            image_gradient_orientation=[gradient_orientation_z_yx_3d, gradient_orientation_yx_3d],
            keypoint=keypoint,
            n_dims=3,
            window_size=16,
            bins=[8, 4]
